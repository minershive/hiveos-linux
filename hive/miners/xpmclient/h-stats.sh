...
get_hashes(){
  ...
  for (( t=0; t < $gpu_count; t++ )); do
    ...
    if [[ ! -z $t_num ]]; then
      t_hs=`cat $log_name | tail -100 | grep INFO | grep "GPU $t_num" | tail -1 | sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g" | cut -d "|" -f 2 | awk '{print $8}' | cut -d "=" -f 2 | cut -d "/" -f 1`
      [[ -z $t_hs ]] && t_hs=0
      hs+="$t_hs "
...
get_miner_uptime(){
  cat $log_name | tail -100 | grep INFO | tail -1 | sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g" | cut -d "(" -f 2 | cut -d "." -f 1
}
...
get_shares(){
  ...
  rej=0
  t=0
  local line=`cat $log_name | tail -100 | grep '(ST/INV/DUP)' | tail -1 | sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g" | cut -d ':' -f 4 `
