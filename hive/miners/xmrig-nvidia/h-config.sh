...
function miner_fork() {
	local MINER_FORK=$XMRIG_NVIDIA_FORK
	[[ -z $MINER_FORK ]] && MINER_FORK=$MINER_DEFAULT_FORK
	echo $MINER_FORK
}

function miner_ver() {
	local MINER_VER=$XMRIG_NVIDIA_VER
...
function miner_config_gen() {
	...
	for url in $XMRIG_NVIDIA_URL; do
		...
		pool=$(cat <<EOF
		{"url": "$url", "user": "$XMRIG_NVIDIA_TEMPLATE", "pass": "$XMRIG_NVIDIA_PASS", "rig-id": "$rig_id", "use_nicehash": $c_nicehash, "tls": $tls, "tls-fingerprint": $tls_fp, "variant": "$variant", "keepalive": true }
EOF
)
