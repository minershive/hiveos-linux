...
{
	int lane = get_local_id(0) % SUBDIV;
...
#pragma unroll
	...
	//	#pragma unroll
	//	for (int i = 1; i < 4; ++i)
	((ulong4*)m)[1] = block[2*lane].lo;
	((ulong4*)m)[2] = block[2*lane].hi;
	((ulong4*)m)[3] = block[1 + 2*lane].lo;
...
#define FARLOAD(x) far[warp][(x) + lane*(2+SHR_OFF)]
#define FARSTORE(x) far[warp][(lane%2) + (x+lane/2)*(2+SHR_OFF)]
#define SHR_OFF 0
#ifdef WORKSIZE
	...
	__global uint4 * Elements, __global uint32_t * __restrict__ SmallestNonce, uint pTarget)
{
	...
	uint32_t thread = get_local_id(0);
	uint32_t NonceIterator = get_global_id(0);
	int lane = get_local_id(0) % SUBDIV;
	int warp = get_local_id(0) / SUBDIV;;//warp_id();
	__local  ulong8 far[TPB_MTP / SUBDIV][SUBDIV * (2 + SHR_OFF)];
	__local  uint32_t farIndex[TPB_MTP / SUBDIV][SUBDIV];
...
//				int lone = lane/4
				...
				for (int t = 0; t<2; t++) {
					...
					__global const ulong8 * __restrict__ farP = (farIndex[warp][t+2*(lane/2)]<half_memcost) ? &DBlock[farIndex[warp][t+2*(lane/2)] * 16 + 2 * i]
						: &DBlock2[(farIndex[warp][t + 2 * (lane / 2)] - half_memcost) * 16 + 2 * i];
				
					far[warp][lane%2 + (t+2*(lane/2)) * (2 + SHR_OFF)] = (last) ? (ulong8)(0, 0, 0, 0, 0, 0, 0, 0) : farP[lane%2];
