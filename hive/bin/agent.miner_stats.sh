...
#				local temp=$(jq -c "[.temp$nvidia_indexes_array]" <<< $gpu_stats)
		...
		xmr-stak)
			...
			if [[ $? -ne 0 || -z $stats_raw ]]; then
				...
				local gpus_disabled=
				(head -n 50 /var/log/miner/xmr-stak/xmr-stak.log | grep -q "WARNING: backend AMD (OpenCL) disabled") && #AMD disabled found
				(head -n 50 /var/log/miner/xmr-stak/xmr-stak.log | grep -q "WARNING: backend NVIDIA disabled") && #and nvidia disabled
				gpus_disabled=1
